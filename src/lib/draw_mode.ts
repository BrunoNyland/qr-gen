...
const draw_label = (ctx: CanvasRenderingContext2D, settings: RequiredQrGenOptions) => {
    const size = settings.size;
    const font = 'bold ' + settings.mSize * 0.01 * size + 'px ' + settings.fontname;

    ctx.strokeStyle = settings.labelStrokeColor || settings.back || '#fff';
    ctx.lineWidth = settings.mSize * 0.01 * size * (settings.labelStrokeWidth || 0.1);
...
const draw_image = (ctx: CanvasRenderingContext2D, settings: RequiredQrGenOptions) => {
...
const draw_mode = (ctx: CanvasRenderingContext2D, settings: RequiredQrGenOptions) => {
